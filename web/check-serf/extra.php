			...
			if($DEBUG) echo "Service: ".$serv->h."<br>";
				foreach ($serv->v as $info) {
					...
					if(empty($graph[$n]))
					 ...
					 $tmpg=$graph[$n];
					array_push($tmpg, array($info->stime, $info->ttime, $serv->h, $info->other));
					$graph[$n]=$tmpg;
				}
			}
		}
...
//			echo "COMP: ".!empty($k)." && ".$k[0]." == ".$s[0]." && ".$k[1]." == ".$s[1]." && ".$k[2]." == ".$s[2]."<br>";
	...
	foreach ($matrix as $key => $lines) {	
	  for ($j=0;$j<count($lines);$j++) {
		...
		if(empty($lines[$j][0]))
		 echo "start: 0,"; //NEED TO CHANGE TO START AT ONE POINT IN TIME instead of NULL
		else
